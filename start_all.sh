...
for host in "${CLIENTS[@]}"; do
  ...
  ssh -o BatchMode=yes "$host" "mkdir -p ~/logs ~/bin >/dev/null 2>&1 || true"

  # b) Client-Skript übertragen, CRLF entfernen, ausführbar machen
  scp -q "$CLIENT_SCRIPT_LOCAL" "$host:~/improved_client.sh"
  ssh -o BatchMode=yes "$host" "
    # CRLF sauber entfernen, falls vorhanden (ohne dos2unix)
    sed -i 's/\r$//' ~/improved_client.sh
    chmod +x ~/improved_client.sh
  "

  # c) Minimal-Check auf benötigte Programme
  MISSING=$(ssh -o BatchMode=yes "$host" "for c in mpv socat inotifywait; do command -v \$c >/dev/null || echo \$c; done")
  if [[ -n "$MISSING" ]]; then
    log "WARN: $host fehlt: $MISSING"
    log "     → Bitte auf $host installieren: sudo apt install -y mpv socat inotify-tools"
  fi

  # d) Vorherige Instanzen beenden
  ssh -o BatchMode=yes "$host" "pkill -f 'improved_client.sh' >/dev/null 2>&1 || true"

  # e) Starten (kein bash -lc, keine komplexen Quotes)
  ssh -o BatchMode=yes "$host" "CLIENT_ID=${CLIENT_ID} nohup ~/improved_client.sh >> ~/logs/client${CLIENT_ID}.log 2>&1 & echo \$! > ~/logs/client${CLIENT_ID}.pid" \
    || log "WARN: $host Startkommando schlug fehl (SSH-Exitcode)"

  # f) Kurz warten & prüfen
  sleep 1
  if ssh -o BatchMode=yes "$host" "pgrep -f 'improved_client.sh' >/dev/null"; then
    log "OK: Client ${CLIENT_ID} läuft"
  else
